...
resource "aws_s3_bucket" "audit" { bucket="${var.name_prefix}-s3-audit-${random_id.suffix.hex}" force_destroy=true tags=merge(var.tags,{Name="${var.name_prefix}-s3-audit"}) }
...
resource "aws_s3_bucket_policy" "audit" { bucket=aws_s3_bucket.audit.id policy=data.aws_iam_policy_document.bucket.json }
resource "aws_cloudtrail" "this" { name="${var.name_prefix}-cloudtrail" s3_bucket_name=aws_s3_bucket.audit.id include_global_service_events=true is_multi_region_trail=true enable_log_file_validation=true depends_on=[aws_s3_bucket_policy.audit] tags=merge(var.tags,{Name="${var.name_prefix}-cloudtrail"}) }
